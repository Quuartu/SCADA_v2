...
public class FileSystemBrowserLogic : BaseNetLogic
{
	public override void Start()
	{
		...
		showHiddenFilesVariable = Owner.Owner.Owner.GetVariable("ShowHiddenFiles");
		if (showHiddenFilesVariable == null)
			throw new CoreConfigurationException("ShowHiddenFiles variable not found");

		accessNetworkDrivesVariable = Owner.Owner.Owner.GetVariable("AccessNetworkDrives");
	...
	private void Browse(ResourceUri resourceUri)
	{
		...
		{
			var backEntry = InformationModel.MakeObject<FileSelectorEntry>("back");
			backEntry.FileName = "..";
	...
	private FileSelectorEntry CreateFilesystemEntry(FileSystemInfo entry, bool isDirectory)
	{
		var fileSystemEntry = InformationModel.MakeObject<FileSelectorEntry>(entry.Name);
